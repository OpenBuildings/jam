...
class Kohana_Jam_Validation_Exception extends Kohana_Exception {
	 ...
	 * @return void
	 */
	public function __construct($alias, Validation $object, $message = 'Failed to validate array: :errors', array $values = NULL, $code = 0)
	{
		$this->_alias = $alias;
		$this->_objects['_object'] = $object;
		$this->_objects['_has_many'] = FALSE;
		$values[':errors'] = print_r($object->errors(), TRUE);
	 ...
	 * @return Jam_Validation_Exception
	 */
	...
	/**
	 * Merges an Jam_Validation_Exception object into the current exception
	 * Useful when you want to combine errors into one array
	 *
	 * @param  Jam_Validation_Exception $object   The exception to merge
	 * @param  mixed                    $has_many The array key to use if this exception can be merged multiple times
	 * @return Jam_Validation_Exception
	 */
	public function merge(Jam_Validation_Exception $object, $has_many = FALSE)
	{
		$alias = $object->alias();
	 ...
	 *
	 *     // Will load Model_User errors from messages/jam-validation/user.php
	 *     $e->errors('jam-validation');
	 *
...
} // End Kohana_Jam_Validation_Exception
