 ...
 */
class Jam_Behavior_CascadeTest extends Unittest_Jam_Database_TestCase {
	...
	public function test_collect_children()
	{
		...
		$this->assertEquals($model, $collected[0]);

		foreach ($model->test_posts as $i => $post) 
		{
			$this->assertEquals($post, $collected[$i+1]);
		}
		...
		{
			$this->assertEquals($item, $collected[$i]);
		}
	}
