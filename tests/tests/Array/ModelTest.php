 ...
 */
class Jam_Array_ModelTest extends Unittest_TestCase {
	...
	public function test_query_builder()
	{
		...
		$this->assertEquals($expected_sql, $this->array->where('name', '=', 'two')->compile());

		$expected_sql = 'SELECT test_elements.* FROM test_elements WHERE test_elements.id = 2 OR test_elements.name = \'one\'';
		$this->assertEquals($expected_sql, $this->array->where('id', '=', 2)->or_where('name', '=', 'one')->compile());
	}
}