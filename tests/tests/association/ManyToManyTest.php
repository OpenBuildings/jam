...
class Jam_Association_ManytomanyTest extends Testcase_Database {
	...
	public function data_join()
	{
		return array(
			...
			array('test_tags', array('join_table' => 'permissions', 'join_table_paranoid' => TRUE), NULL, NULL, 'JOIN `permissions` ON (`permissions`.`test_blog_id` = `test_blogs`.`id` AND `permissions`.`is_deleted` IS NULL) JOIN `test_tags` ON (`test_tags`.`id` = `permissions`.`test_tag_id`)'),
			array('test_tags', array('join_table' => 'permissions', 'join_table_paranoid' => 'deleted'), NULL, NULL, 'JOIN `permissions` ON (`permissions`.`test_blog_id` = `test_blogs`.`id` AND `permissions`.`deleted` IS NULL) JOIN `test_tags` ON (`test_tags`.`id` = `permissions`.`test_tag_id`)'),
	...
	public function data_get()
	{
		return array(
			...
			array('test_tags', array('join_table' => 'permissions', 'join_table_paranoid' => TRUE), NULL, NULL, 'SELECT `test_tags`.* FROM `test_tags` JOIN `permissions` ON (`permissions`.`test_tag_id` = `test_tags`.`id`) WHERE `permissions`.`test_blog_id` = 1 AND `permissions`.`is_deleted` IS NULL'),
			array('test_tags', array('join_table' => 'permissions', 'join_table_paranoid' => 'deleted'), NULL, NULL, 'SELECT `test_tags`.* FROM `test_tags` JOIN `permissions` ON (`permissions`.`test_tag_id` = `test_tags`.`id`) WHERE `permissions`.`test_blog_id` = 1 AND `permissions`.`deleted` IS NULL'),
